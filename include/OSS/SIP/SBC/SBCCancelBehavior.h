...
#ifndef SIP_SBCCANCELBEHAV_INCLUDED
#define SIP_SBCCANCELBEHAV_INCLUDED
...
class OSS_API SBCCancelBehavior : public SBCDefaultBehavior
{
public:
  SBCCancelBehavior(SBCManager* pManager);
  ...
  void registerInvite(const std::string& id, const SIPMessage::Ptr& pInvite);
    /// Register an INVITE in the cancellable pool

  void removeInvite(const std::string& id);
    /// Remove the invite from the canellable pool
...
#endif // SIP_SBCCANCELBEHAV_INCLUDED

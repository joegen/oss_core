...
TEST(Net, RTNLRoute)
{
  ...
  for (OSS::Net::RTNLRoutes::const_iterator iter = routes.begin(); iter != routes.end(); iter++)
  {
    std::cout << 
      "Dest: " << iter->destination << std::endl <<
      "Device: " << iter->device << std::endl <<
      "Source: " << iter->source << std::endl <<
      "Gateway: " << iter->gateway << std::endl << std::endl;
  }

  std::string source;

  ASSERT_TRUE(OSS::Net::rtnl_get_source(routes, source, "8.8.8.8", true));
  std::cout << "Source: " << source << " Target: 8.8.8.8" << std::endl;
  
  ASSERT_TRUE(OSS::Net::rtnl_get_source(routes, source, "192.168.122.25", true));
  std::cout << "Source: " << source << " Target: 192.168.122.25" << std::endl;
  
  ASSERT_TRUE(OSS::Net::rtnl_get_source(routes, source, "192.168.0.100", true));
  std::cout << "Source: " << source << " Target: 192.168.0.100" << std::endl;
  
  ASSERT_TRUE(OSS::Net::rtnl_get_source(routes, source, "10.10.10.10", true));
  std::cout << "Source: " << source << " Target: 10.10.10.10" << std::endl;

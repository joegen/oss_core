...
exports.sbc_export_global_vars = function(sipMessage, global) {
  ...
  // Transcoder
  //
  if (_hook && _hook.sbc_is_transcoding_enabled()) {
    if (global.sbc_send_ptime_attribute !== null) {
      if (global.sbc_send_ptime_attribute) {
        sipMessage.setTransactionProperty("send-ptime", "1");
      } else {
        sipMessage.setTransactionProperty("send-ptime", "0");
      }
    }

    if (global.sbc_send_ssrc_attribute !== null) {
      if (global.sbc_send_ssrc_attribute) {
        sipMessage.setTransactionProperty("send-ssrc", "1");
      } else {
        sipMessage.setTransactionProperty("send-ssrc", "0");
      }
    }

    if (global.sbc_offer_secure_rtp) {
      sipMessage.setTransactionProperty("offer-secure-rtp", "1");
      global.sbc_enable_transcoding = true;
    }

    if (global.sbc_enable_transcoding) {
      sipMessage.setTransactionProperty("enable-transcoding", "1");
      if (global.sbc_target_behind_nat) {
        sipMessage.setTransactionProperty("target-behind-nat", "1");
      }

      if (global.sbc_caller_behind_nat) {
        sipMessage.setTransactionProperty("caller-behind-nat", "1");
      }
      var i = 0;
      var strCodecs = "";

      if (global.sbc_inbound_codecs.length) {
        for (i = 0; i < global.sbc_inbound_codecs.length; i++) {
          strCodecs += global.sbc_inbound_codecs[i];
          if (i < global.sbc_inbound_codecs.length - 1) {
            strCodecs += ",";
          }
        }

        if (strCodecs.length) {
          sipMessage.setTransactionProperty("inbound-codecs", strCodecs);
        }
      }

      if (global.sbc_outbound_codecs.length) {
        i = 0;
        strCodecs = "";
        for (i = 0; i < global.sbc_outbound_codecs.length; i++) {
          strCodecs += global.sbc_outbound_codecs[i];
          if (i < global.sbc_outbound_codecs.length - 1) {
            strCodecs += ",";
          }
        }

        if (strCodecs.length) {
          sipMessage.setTransactionProperty("outbound-codecs", strCodecs);
        }
      }
    }
  }

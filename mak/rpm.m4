...
if test -z "$CENTOS_BASE_URL"; then
 ...
 See http://wiki.ossapp.com/display/oss/Install+squid+caching+server+to+reduce+build+time for more details.])
  fi
fi
...
AllDistrosDefault="centos-5-i386 centos-5-x86_64 centos-6-i386 centos-6-x86_64 fedora-16-i386 fedora-16-x86_64 fedora-17-i386 fedora-17-x86_64"
AC_ARG_VAR(ALL_DISTROS, [All distros which using cross distroy compiling (xc.* targets) Default is ${AllDistrosDefault}])
ALL_DISTROS="${AllDistrosDefault}"
...
[
  ...
  AC_ARG_VAR(DOWNLOAD_LIB_URL, [When to cache source files that are downloaded, default download.ossapp.com])
  test -n "${DOWNLOAD_LIB_URL}" || DOWNLOAD_LIB_URL=http://download.ossapp.com/pub/oss/libs
  ...
  AC_ARG_ENABLE(pull-missing-rpms,
     AC_HELP_STRING([--disable-pull-missing-rpms], [Do not use bridge.ossapp.com for rpms that are not built locally]))
